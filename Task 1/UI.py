...
def run_model():
    ...
    accuracy = Training(feature1+1, feature2+1, label1, label2, float(eta_entry.get())
             , int(epochs_entry.get()), b)
    result = userTesting(float(feature1_entry.get()),float(feature2_entry.get()),b)

    create_label("The Model Accuracy is: "+ str(accuracy), 8, 1)
    create_label("Class ID: "+str(result),9,1)
...
create_label("Enter 1st Feature Value: ", 6, 0)
feature1_entry = Entry(window, bd=5)
feature1_entry.grid(row=6, column=1)

create_label("Enter 2nd Feature Value: ", 7, 0)
feature2_entry = Entry(window, bd=5)
feature2_entry.grid(row=7, column=1)
